...
format:
  revealjs:
    ...
    slide-number: true
    transition: fade
    logo: macrolides-images/DMM_newlogo.png
    width: 1280
    height: 720
    lightbox: true
...
::: {.notes}
This is a 1.5-hour lecture. We'll keep moving. Stop me at any point. The macrolides are deceptively simple to prescribe but mechanistically rich — I want you to leave with a working mental model of *why* azithromycin behaves so differently from erythromycin, *why* the D-test exists, and *why* we still add clindamycin to invasive group A strep infections.
:::
...
::: {.notes}
The defining feature is the macrolactone ring of 14, 15, or 16 atoms, with one or more sugars attached. The 3-keto substitution that defines the ketolides was designed specifically to overcome MLS~B~ resistance — we'll see how well that worked. Lincosamides bear no chemical resemblance to macrolides but bind so close to the same ribosomal site that they cross-resist with them.
:::

## Mechanism of Action
<br>

- Bind the **50S ribosomal subunit** at the **peptidyl transferase center** of 23S rRNA (domain V)
- Contact site: nucleotides **A2058–A2059** of 23S rRNA (*E. coli* numbering)
- Block the nascent peptide exit tunnel → **inhibit peptide bond elongation** and trigger premature drop-off of peptidyl-tRNA
- Predominantly **bacteriostatic** at clinically achievable concentrations; bactericidal against highly susceptible organisms (e.g., *S. pyogenes*) at higher drug:bug ratios
...
::: {.notes}
A2058 is the keystone. It's the same nucleotide modified by erm methylases and mutated in azithromycin-resistant Mycoplasma and gonorrhea. Knowing where the drug sits on the ribosome is what lets you predict every resistance mechanism we're about to discuss.
:::
...
::: aside
[@Mao1968; @Ackermann2003]
:::

::: {.notes}
Three mechanisms — but the one that haunts the consult service is target-site methylation, because of MLS~B~ cross-resistance. The efflux phenotype is more common in pneumococci and spares clindamycin. Enzymatic destruction is rarely clinically important except in some Enterobacterales.
:::

## Target Methylation — the *erm* Genes
<br>

- *erm* (erythromycin ribosome methylase) genes encode methyltransferases that **mono- or di-methylate adenine A2058** of 23S rRNA
- Methylation blocks binding of macrolides, lincosamides, *and* streptogramin B → **MLS~B~ phenotype**
- Common variants:
  - **ermB** — broad host range; predominant in pneumococci, GAS, enterococci
  - **ermA, ermC** — staphylococci
  - **ermF** — Bacteroides
- Carried on plasmids and transposons; mobile
...
::: {.notes}
*erm* is the single most important resistance determinant for this drug class. The gene is mobile, the modification is one chemical step, and it knocks out every drug that touches A2058 — that's why clindamycin loses too even though it's not a macrolide.
:::
...
::: {.notes}
This is the classic teaching pearl. AST will say "clindamycin susceptible" but the patient relapses, and you reculture and find a constitutively resistant organism. The mechanism is real — Siberry described it in MRSA, but it applies equally to GAS.
:::
...
::: {.notes}
Every micro lab in Italy and the US does this routinely. If the report says "D-test positive" or "inducible clindamycin resistance" — clindamycin is OFF the table for that isolate, even though the MIC reading was in the susceptible range.
:::
...
::: aside
[@Kawai2013; @Nash1995; @Ng2002; @Meier1996]
:::

::: {.notes}
Mycoplasma macrolide resistance is now >90% in some Asian regions and approaching 30% in parts of Italy — relevant for empiric atypical coverage. H. pylori clarithromycin resistance is the reason quadruple therapy is now first-line in many regions instead of standard triple.
:::
...
: Erythromycin formulations {#tbl-eryth-prep}
...
::: aside
[@Cooper2002; @SanFilippo1976]
:::

::: {.notes}
The motilin agonism is why we used IV erythromycin for gastroparesis. The pyloric stenosis association in neonates is a teaching board favorite — it's a real risk in the first 2 weeks of life, with relative risk around 8–10 in case-control series.
:::
...
::: {.notes}
For pregnant patients needing macrolide therapy, azithromycin is essentially universally first choice — safety data are the best of the class and tolerability is excellent. Clarithromycin gets avoided where possible.
:::
...
: Macrolide comparison {#tbl-macro-compare}

::: {.notes}
If you remember nothing else from the PK section, remember this table. It explains why azithromycin replaced erythromycin in almost every outpatient indication.
:::
...
- Macrolides block the **hERG (IKr) cardiac potassium channel** → prolonged repolarization
- Effect is **dose-dependent** and additive with other QT-prolonging drugs
- Greater risk with:
  - IV administration / rapid infusion
  - Underlying long QT, structural heart disease
  - Hypokalemia, hypomagnesemia
  - Concurrent QT-prolonging drugs (antipsychotics, fluoroquinolones, ondansetron, methadone)
...
::: aside
[@Ray2004; @Ray2012; @Albert2014]
:::

::: {.notes}
These three papers reshaped FDA labeling. The absolute risk is small for the average patient but concentrates dramatically in those with structural heart disease, electrolyte derangements, or other QT drugs. Worth a mental checklist before prescribing azithromycin to an older or polypharmacy patient.
:::
...
::: aside
[@Pejcic2021; @Nappe2016]
:::

::: {.notes}
The "macrolide allergy" label gets attached very loosely in records. Most patients can tolerate alternatives within the class, and clindamycin is safe to use despite being a 50S-binding cousin.
:::
...
::: {.notes}
Practical: I rarely use erythromycin IV in ICU patients without checking baseline QTc and electrolytes. Azithromycin is safer, but not exempt.
:::
...
::: {.notes}
The intracellular niche is where macrolides shine. *M. tuberculosis* is intrinsically resistant — don't confuse it with MAC, which is fully susceptible.
:::
...
::: {.notes}
This is an exam favorite — students assume "macrolides cover mycobacteria" but TB is the standout exception. The Erm(37) methylase is intrinsic to the species.
:::
...
::: {.notes}
The geographic skew matters. Italian and Southern European pneumococcal resistance leans toward *ermB* with high MICs and full MLS~B~ cross-resistance — qualitatively worse than the US efflux pattern.
:::
...
::: {.notes}
This is one of the most important geographic distinctions in antimicrobial therapy. The European/Italian epidemiology favors high-level MLS~B~ resistance — when you read US guidelines that recommend monotherapy macrolide for outpatient pneumonia, mentally adjust for our resistance landscape.
:::
...
::: {.notes}
The macrolide adds atypical coverage and possibly immunomodulation. Several observational studies suggest mortality benefit from combination β-lactam + macrolide even when no atypical is identified — possibly an immunomodulatory effect, possibly residual confounding.
:::
...
::: aside
[@Critchley2002; @Kawai2013]
:::

::: {.notes}
For severe Legionella, most experts prefer levofloxacin over azithromycin, based on observational data — though the comparison has never been done in a proper RCT.
:::
